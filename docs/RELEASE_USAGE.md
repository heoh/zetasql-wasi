...
### 1. `zetasql_local_service_wasi.wasm`
...
### 2. `zetasql_local_service_proto.tar.gz`
...
- `zetasql_local_service_wasi.wasm.sha256`
- `zetasql_local_service_proto.tar.gz.sha256`
...
1. **Extract Protos**: Extract `zetasql_local_service_proto.tar.gz` to get the `.proto` files.
2. **Generate Code**: Use `protoc` to generate code for your target language.
3. **Load WASM**: Use a WASM runtime to load `zetasql_local_service_wasi.wasm`.
