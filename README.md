...
When you are done, run the export script to generate patch files from your commits and update the Bazel configuration:
...
- Update `patches/patches.bzl` so Bazel knows about them.

### 4. Build

Now you can build the project with your changes applied:
...
2. Update the `COMMIT_HASH` variable in `WORKSPACE`.
3. Run `./scripts/setup_dev.sh` to reset the workspace and attempt to re-apply patches.
4. Resolve any merge conflicts in `_work/zetasql`, commit the fixes, and run `./scripts/export_patches.sh`.
